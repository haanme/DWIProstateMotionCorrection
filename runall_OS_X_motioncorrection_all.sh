...
binaryname3="python motioncorrection_DICOM2animatedGIF.py"
selectionfile="namelist_1.txt"
...
do
    ...
    xtermcmd=$(echo "$binaryname --dicomdir $f/DICOMconverted --subject $subject_id --coreg_rigid $param_rigid --coreg_nonrigid $param_BSpline --orig_subdir Noncorrected10")
    echo $xtermcmd
    ret=$(eval $xtermcmd)
    if [ "$?" -eq "0" ]
    then
    ...
    xtermcmd=$(echo "$binaryname2 --dicomdir $f/DICOMconverted --subject $subject_id --outdir $output_dir --orig_subdir Noncorrected10 --subregionfilename subregion10.txt")
    echo $xtermcmd
    ret=$(eval $xtermcmd)
    if [ "$?" -eq "0" ]
    then
    ...
    xtermcmd=$(echo "$binaryname3 --subject $subject_id --outdir $output_dir --slice 11 --orig_subdir Noncorrected10")
    echo $xtermcmd
    ret=$(eval $xtermcmd)
    if [ "$?" -eq "0" ]
    then
